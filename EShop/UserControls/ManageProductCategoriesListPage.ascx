<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ManageProductCategoriesListPage.ascx.cs" Inherits="EShop.UserControls.ManageProductCategoriesListPage" %>
...
<br />
<div align="center">
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ConnStringStoreDB %>" DeleteCommand="DELETE FROM [products] WHERE [prod_id] = @prod_id" InsertCommand="INSERT INTO [products] ([cat_id], [prod_title], [prod_price], [prod_desc], [prod_image]) VALUES (@cat_id, @prod_title, @prod_price, @prod_desc, @prod_image)" SelectCommand="SELECT * FROM [products] WHERE ([cat_id] = @cat_id)" UpdateCommand="UPDATE [products] SET [cat_id] = @cat_id, [prod_title] = @prod_title, [prod_price] = @prod_price, [prod_desc] = @prod_desc, [prod_image] = @prod_image WHERE [prod_id] = @prod_id">
    ...
    <asp:GridView ID="GridView1" runat="server" AllowPaging="True" AutoGenerateColumns="False" DataKeyNames="prod_id" DataSourceID="SqlDataSource1" PageSize="8">
        <Columns>
            ...
            <asp:BoundField DataField="prod_price" DataFormatString="RM {0:n2}"
                HeaderText="Product Price" SortExpression="prod_price">
            ...
            <asp:BoundField DataField="prod_image" HeaderText="Image Filename"
